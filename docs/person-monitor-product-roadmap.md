...
- Manual Review Queue F2a–F2e готова: stable logical tasks + snapshot occurrences у Neon, atomic reference-only store/sync, orchestrator/API production wiring, analyst status API та portal analyst UI.
...
- Canonical PDF та Excel exports завершені: обидва будуються через shared safe `dossier-export-model-v1` з exact persisted `dossier_versions` snapshot; endpoints приймають лише `dossierVersionId`, а frontend експортує саме поточну відображену версію. Legacy report exports поки лишаються compatibility path.
...
- Обидва endpoints мають `Cache-Control: no-store`, відхиляють `subjectId` та не запускають live dossier rebuild.
...
2. **Orchestrator «Сформувати досьє».** Explicit analyst action `Сформувати / Оновити досьє` завершена: лише ця UI-дія запускає `POST /api/dossier`; після успішного persistence portal перечитує latest immutable snapshot через read API. Автоматичної генерації при виборі subject або відкритті review snapshot немає.
...
5. **Версії досьє та audit trail.** `dossier_versions`, canonical payload hash, insert-only store, production orchestrator wiring, live Neon snapshot verification та latest/exact snapshot read уже реалізовані. Manual Review UI використовує `dossier_versions.id` для exact snapshot navigation. Далі — version history, run/source metadata та порівняння змін між версіями; `report_id` у `report-model-v1` лишається reserved nullable field.
...
Не підключати нові великі джерела одразу. Core evidence/provenance E1–E3, final dossier presentation G1–G7 та explicit analyst build/update action уже завершено. Поточна оптимальна послідовність: **canonical PDF/Excel → audit/diff → після цього AUTO.RIA / нерухомість / OpenDataBot.**
