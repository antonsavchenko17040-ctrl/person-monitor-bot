...
**Технічний стан:** повний regression suite GREEN. Phase 1 research entry та identity clarification foundation реалізовані: початкова форма приймає лише ПІБ, створює persisted `research_request`, формує safe candidate payload і пропонує організацію, посаду, місто та дату народження у випадайках зі знайдених кандидатів. Refinement повторно виконує explainable identity resolution у межах того самого request; manual candidate accept/reject збережені. Дата народження враховується як strong signal та explicit conflict. Зовнішній API contract: `POST /api/research`, `GET /api/research-status`, `POST /api/research-refine`, `POST /api/research-resolve`. Існуючі dossier versions, Manual Review, evidence UI, exact-version exports та explicit dossier build flow збережені без автоматичного rebuild на page load.
