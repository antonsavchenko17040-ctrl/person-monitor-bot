...
5. **Версії досьє та audit trail.** `dossier_versions`, canonical payload hash, insert-only store, production orchestrator wiring і live Neon snapshot verification уже реалізовані. Далі — `report_id` semantics, run/source metadata та порівняння змін між версіями.
